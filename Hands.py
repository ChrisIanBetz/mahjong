...
def hand_distance(rack: [[str], [str], [str], [str]], hand: [[str], [str], [str], [str]]):
    ...
    #Check rack size
    rack_size = len(rack[0]) + len(rack[1]) + len(rack[2]) + len(rack[3])
    if rack_size != 13 and rack_size != 14:
        raise Exception("Invalid rack size of " + str(rack_size))
